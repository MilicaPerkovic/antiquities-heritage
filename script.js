...
// === Radio exhibition gallery (archive.html) — dedicated lightbox (no caption) ===
const radioGallery = document.querySelector('.radio-gallery');
const radioGallerySources = Array.from(
  document.querySelectorAll('.radio-gallery-source img'),
);
const radioGalleryThumbs = Array.from(
  document.querySelectorAll('[data-radio-gallery-index]'),
);

if (radioGallery && radioGallerySources.length) {
  let radioLightboxEl = null;
  let radioIndex = 0;

  function buildRadioLightbox() {
    if (radioLightboxEl) return;
    radioLightboxEl = document.createElement('div');
    radioLightboxEl.className = 'radio-lightbox';
    radioLightboxEl.setAttribute('role', 'dialog');
    radioLightboxEl.setAttribute('aria-modal', 'true');
    radioLightboxEl.setAttribute('aria-label', 'Pregled fotografije');
    radioLightboxEl.innerHTML = `
      <button class="radio-lightbox-close" type="button" aria-label="Zatvori">&times;</button>
      <button class="radio-lightbox-prev" type="button" aria-label="Prethodna">&lsaquo;</button>
      <img class="radio-lightbox-img" src="" alt="" />
      <button class="radio-lightbox-next" type="button" aria-label="Sljedeća">&rsaquo;</button>
    `;
    document.body.appendChild(radioLightboxEl);

    radioLightboxEl
      .querySelector('.radio-lightbox-close')
      .addEventListener('click', closeRadioLightbox);
    radioLightboxEl
      .querySelector('.radio-lightbox-prev')
      .addEventListener('click', () => stepRadio(-1));
    radioLightboxEl
      .querySelector('.radio-lightbox-next')
      .addEventListener('click', () => stepRadio(1));
    radioLightboxEl.addEventListener('click', (event) => {
      if (event.target === radioLightboxEl) closeRadioLightbox();
    });
  }

  function showRadioImage() {
    const img = radioGallerySources[radioIndex];
    radioLightboxEl.querySelector('.radio-lightbox-img').src =
      img.getAttribute('src');
    radioLightboxEl.querySelector('.radio-lightbox-img').alt = img.alt || '';
  }

  function openRadioLightbox(index) {
    if (!radioGallerySources.length) return;
    buildRadioLightbox();
    radioIndex = (index + radioGallerySources.length) % radioGallerySources.length;
    showRadioImage();
    radioLightboxEl.classList.add('is-open');
    document.body.classList.add('no-scroll');
  }

  function closeRadioLightbox() {
    if (!radioLightboxEl) return;
    radioLightboxEl.classList.remove('is-open');
    document.body.classList.remove('no-scroll');
  }
  ...
  function stepRadio(direction) {
    if (!radioGallerySources.length) return;
    radioIndex =
      (radioIndex + direction + radioGallerySources.length) %
      radioGallerySources.length;
    showRadioImage();
  }

  radioGalleryThumbs.forEach((thumb) => {
    thumb.addEventListener('click', (event) => {
      event.preventDefault();
      const idx = parseInt(thumb.getAttribute('data-radio-gallery-index'), 10);
      if (Number.isInteger(idx)) openRadioLightbox(idx);
    });
  });

  document.addEventListener('keydown', (event) => {
    if (!radioLightboxEl || !radioLightboxEl.classList.contains('is-open')) return;
    if (event.key === 'Escape') closeRadioLightbox();
    if (event.key === 'ArrowLeft') stepRadio(-1);
    if (event.key === 'ArrowRight') stepRadio(1);
  });
}
